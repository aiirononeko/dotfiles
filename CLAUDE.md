...
Neovim + シェル の設定ファイル(dotfiles)を管理するリポジトリ。macOS / Linux (WSL2) / Windows の3環境に対応。
...
`install.ps1` は以下をシンボリックリンクで配置する:
- `nvim/` → `%LOCALAPPDATA%\nvim`
- `.ps_profile.ps1` → `$PROFILE.CurrentUserCurrentHost`
- `im-select.exe` のインストール（winget → GitHub Releases フォールバック）

シンボリックリンク作成には開発者モードまたは管理者権限が必要。失敗時はコピーにフォールバック。
...
- `.ps_profile.ps1` は Windows (PowerShell) 用プロファイル。`.zshrc` と同等の設定を PowerShell 向けに実装
- `core/ime.lua` は macOS / WSL / Windows native の3環境に対応。Windows native では `im-select.exe` を使用
